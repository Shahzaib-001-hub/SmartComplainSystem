    ...
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 px-4 sm:px-6 backdrop-blur-md transition-colors">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3">
        ...
        <div className="hidden sm:flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80">
          <FontAwesomeIcon
            icon={isAdmin ? faUserTie : faUserGraduate}
            className={isAdmin ? 'text-rose-600 dark:text-rose-400' : 'text-indigo-600 dark:text-indigo-400'}
          />
          <span className={isAdmin ? 'text-rose-600 dark:text-rose-400 font-bold' : 'text-indigo-600 dark:text-indigo-400 font-bold'}>
        ...
        <div className="relative">
          ...
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-600/15 text-indigo-600 dark:bg-indigo-600/20 dark:text-indigo-400 border border-indigo-500/30 font-bold text-sm">
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-56 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl shadow-black/20 dark:shadow-black/50 py-2 z-50 animate-scale-in">
                <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-800">
                  ...
                  {user?.department && (
                    <span className="inline-block mt-1 text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                      {user.department} {user.studentId ? `(${user.studentId})` : ''}
                    </span>
                  )}
                </div>
